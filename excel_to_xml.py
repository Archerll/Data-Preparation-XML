...
def check_curr(currCode, balance , payment):
    if currCode == 'JPY':
        if re.search('\.', balance) or re.search('\.', payment):
            messagebox.showinfo("警告！", "数据的日元金额数值非法 ，请重新执行程序")
    else:
        if re.search('\.([0-9])', balance) or re.search('\.([0-9])'):
            messagebox.showinfo("警告！", "数据的金额数值非法 ，请重新执行程序")
...
def insert_xml():
    with open('FATCA.xml', 'r+', encoding='utf8') as temp_xml:
        ...
        path = 'final_file\\{0}_Payload.xml'.format(FATCAEntitySenderId)
...
class WriteXml(object):
    ...
    def create_control_xml(self, data):
        ...
        for i in range(count):
            ...
            Individual = self.doc.createElement('ftc:Individual')
